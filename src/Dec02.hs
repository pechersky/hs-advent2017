...
sumspreadsheet :: T.Text -> Integer
sumspreadsheet = (sum . (fmap (diffrow . intarr)) . cell . rows)
  where
    rows :: T.Text -> [T.Text]
    rows = T.splitOn "  "
    ...
    intarr :: [T.Text] -> [Integer]
    intarr = fmap (read . T.unpack)

    diffrow :: [Integer] -> Integer
    diffrow row = (maximum row) - (minimum row)


sumspreadsheet2 :: T.Text -> Integer
sumspreadsheet2 = (sum . (fmap (finishrow . intarr)) . cell . rows)
  where
    rows :: T.Text -> [T.Text]
    rows = T.splitOn "  "
